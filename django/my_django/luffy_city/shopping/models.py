...
class Coupon(models.Model):
    ...
    # 通用券 只录入等值货币字段
    # 满减券  录 最低消费 等值货币
    # 折扣券  根据折扣券不同可以录入 折扣百分比  还可以录入最低消费
    ...
    open_date = models.DateTimeField("优惠券领取开始时间")
    close_date = models.DateTimeField("优惠券领取结束时间")
    ...
    coupon_valid_days = models.PositiveIntegerField(verbose_name="优惠券有效期（天）", blank=True, null=True,
                                                    help_text="自券被领时开始算起")
    date = models.DateTimeField(auto_now_add=True) #添加后不会变
